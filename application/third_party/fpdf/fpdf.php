...
function plot_table($widths, $lineheight, $table, $border=1, $aligns=array(), $fills=array(), $links=array()){
	$func = function($text, $widths){
		$len=strlen($text);
		$twidth = $this->GetStringWidth($text);
		$split = floor($widths * $len / $twidth);
		$w_text = explode( "\n", wordwrap( $text, $split, "\n", true));
		return $w_text;
	};
	foreach ($table as $line){
		...
		foreach ($line as $key => $cell){
			...
			foreach ($cell as $textline){
				//print_r("ancho:".$widths[$key]." alto:".$height." texto:".$textline);exit();
			...
			//print_r("ancho:".$widths[$key]." alto:".$lineheight * $maxlines." texto:".$textline);exit();
